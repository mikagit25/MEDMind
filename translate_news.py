...
SUMMARISE_SYS = """\
You are a medical science journalist writing for healthcare professionals.
Given a title and abstract, write a concise summary (250-350 words) in clear, engaging English prose.
Structure (no headers, no bullets):
1. Lead sentence with key finding
2. Study context: design, population (2 sentences)
3. Main results with numbers
4. Clinical significance
5. One-line caveat if relevant
...
def _resummarise(title: str, abstract: str) -> str:
    user = f"Title: {title}\n\nAbstract:\n{abstract}"
    return _ai(SUMMARISE_SYS, user, max_tokens=600)


# ── Main logic ────────────────────────────────────────────────────────────────

def run_translate(conn, dry_run: bool = False):
...
def run_resummarise(conn, dry_run: bool = False):
    """Re-summarise articles where summary looks like a raw abstract."""
    cur = conn.cursor()
    cur.execute("""
    ...
    """)
    rows = cur.fetchall()
    log.info("Articles with raw abstracts to re-summarise: %d", len(rows))
...
def main():
    ...
    if args.all or args.resummarise:
        ...
        run_resummarise(conn, dry_run=args.dry_run)

    if args.all or args.translate:
        log.info("=== PHASE 2: Translate articles ===")
        run_translate(conn, dry_run=args.dry_run)
