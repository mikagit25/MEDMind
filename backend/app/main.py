...
async def lifespan(app: FastAPI):
    ...
    try:
        await get_redis()  # Initialize Redis connection (non-fatal if unavailable)
    except Exception as e:
...
app.include_router(product_analytics.router, prefix=API_PREFIX)
...
app.include_router(pricing.router, prefix=API_PREFIX)  # G3 — regional pricing
app.include_router(exam_outcomes.router, prefix="")    # V7 Phase 3 — post-exam survey
...
app.include_router(reviewer_queue.router, prefix=API_PREFIX)
app.include_router(calculators.router, prefix=API_PREFIX)
app.include_router(telegram.router, prefix=API_PREFIX)
app.include_router(health_profile.router, prefix=API_PREFIX)
