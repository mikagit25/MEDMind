#!/usr/bin/env python3
"""
MedMind AI — Daily Arabic NCLEX Q&A Shorts (AR channel)

Reads translated questions from nclex_shorts_translations (lang='ar'),
renders 9:16 video with Arabic reshaping, uploads to Arabic YouTube account.

Tracking: /opt/medmind/youtube_shorts_nclex_ar_uploaded.json
Cron:     10 21 * * *  (21:10 UTC — after AR article videos at 20:45)
"""
...
from nclex_to_shorts import build_video  # noqa: E402
...
TOKEN_FILE    = Path(os.environ.get("YT_TOKEN",         "/opt/medmind/youtube_token_ar.json"))
SECRET_FILE   = Path(os.environ.get("YT_CLIENT_SECRET", "/opt/medmind/client_secret_account3_web.json"))
TRACKING_FILE = Path(os.environ.get("YT_NCLEX_AR_TRACKING",
                                    "/opt/medmind/youtube_shorts_nclex_ar_uploaded.json"))

DB_URL = os.environ.get("DB_URL", "postgresql://medmind:medmind_secret@localhost:5432/medmind")

HASHTAGS = (
    "#NCLEX #تمريض #طلاب_التمريض #اختبار_التمريض "
    "#MedMindAI #Shorts #تعليم_طبي #دراسة_التمريض"
)
...
def _save_ar_tracking(question_id: str, yt_id: str | None = None):
...
def _yt_upload(mp4: str, title: str, description: str) -> str | None:
    ...
    body = {
        "snippet": {
            ...
            "tags":       ["nclex", "تمريض", "طلاب التمريض", "nursing", "medmind",
                           "تعليم طبي", "shorts", "اختبار تمريض"],
            "categoryId": "27",
            "defaultLanguage": "ar",
        },
        "status": {
...
def _make_title(q: dict) -> str:
    diff_ar = {"easy": "سهل", "medium": "متوسط", "hard": "صعب"}.get(
        q.get("difficulty", "medium"), "متوسط"
    )
    return f"🩺 سؤال NCLEX للتمريض ({diff_ar}) #Shorts"[:100]


def _make_description(q: dict) -> str:
    opts = q["options"] if isinstance(q["options"], dict) else json.loads(q["options"])
    return (
        f"هل يمكنك الإجابة على هذا السؤال؟\n\n"
        f"❓ {q['question'][:200]}\n\n"
        f"A) {opts.get('A', '')}\n"
        f"B) {opts.get('B', '')}\n"
        f"C) {opts.get('C', '')}\n"
        ...
        f"تدرب على أكثر من 1,000 سؤال NCLEX مجاناً:\n"
...
async def run(limit: int, dry_run: bool):
    ...
    if not questions:
        print("✅ No new AR NCLEX questions to upload today.")
        return

    print(f"📋 Found {len(questions)} question(s) for AR channel (limit={limit})")
    ...
    print(f"\n{'='*50}")
    print(f"Done: {uploaded_count}/{min(len(questions), limit)} uploaded to AR channel")
